...
contract MultiStageAuction {
    ...
    constructor() {
        auctioneer = msg.sender; // Ideally only the auctioneer should control phases.
    }

    function startAuction() public {
        require(
            currentPhase == AuctionPhase.Commit,
            "Auction cannot be started again."
        );
        currentPhase = AuctionPhase.Commit;
    }
    ...
    // Vulnerability: Allows resetting auction phase to Commit indiscriminately
    function resetAuctionPhase() public {
        require(
            currentPhase == AuctionPhase.Ended,
            "Auction has not ended yet."
        );
        currentPhase = AuctionPhase.Commit; // Resets to Commit, allowing new bids.
    }
