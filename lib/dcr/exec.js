...
class DCRExecutor {
  ...
  async executeActivity(dcrId, simId, activityId, activityValue = null, activityType = null, dcrRole = null) {
    ...
    try {
      ...
      execLogger.info(`  Response data: ${JSON.stringify(res.data)}`);
      execLogger.info(`  Response status: ${res.status}`);
      execLogger.info(`  Response headers: ${JSON.stringify(res.headers)}`);
      ...
      // }
      execLogger.error(`  Error response data: ${JSON.stringify(error.response.data)}`);
      execLogger.error(`  Error response status: ${error.response.status}`);
      execLogger.error(`  Error response headers: ${JSON.stringify(error.response.headers)}`);
