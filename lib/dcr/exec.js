...
class DCRExecutor {
  ...
  async executeActivity(dcrId, simId, activityId, activityValue = null, activityType = null, dcrRole = null) {
    ...
    try {
      logger.debug('Trying to execute the following DCR activity');
      logger.debug(`requestBody: ${JSON.stringify(requestBody, null, 2)}`);
      logger.debug(`nextEventAddress: ${nextEventAddress}`);
