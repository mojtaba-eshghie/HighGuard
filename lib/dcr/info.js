      ...
      return new Promise((resolve, reject) => {
        xml2js.parseString(response.data, (err, result) => {
          ...
          try {
            ...
            //     infoLogger.debug(`this the subprocess event: ${JSON.stringify(event.event)}`);
            //     return event.event.map(e => e.$.id);
            //   }
            //   return event.$.id;
            // });
            ...
            infoLogger.debug(`Found the activities: ${JSON.stringify(eventIds)}`);
            resolve(eventIds);
          } catch (error) {
...
const getPendingActivities = (modelID, simID) => {
  ...
  return unsafeAuth().then(basicAuthorization => {
    ...
    infoLogger.debug(`the url is ${url}`);
