 ...
 */
async function compileWithVersion(source, contractFileName, version) {
    ...
    const contract = output.contracts[fileName][contractFileName];
    return {
        abi: contract.abi,
...
let getContractABI = async (contractFileName) => {  
    try {
        ...
        const compiledContract = await compileWithVersion(contractSource, contractFileName, solcVersion);
    ...
    constructorParamSpecs.forEach((item, index, array) => {
        if (item.sourceType == "dynamic") {
            if (item.source.type == "EOA") {
                ...
                logger.error(`Contract constructor parameter source type type not yet implemented`);
                throw new Error(`Contract constructor parameter source type type not yet implemented`)
            } else {
                logger.error(`Constructor parameter specs source type is neither "EOA" nor "contract`);
                throw new Error('Constructor parameter specs source type is neither "EOA" nor "contract');
            }
        } else if (item.sourceType == "static") {
            logger.error(`Static constructor parameter type type not yet implemented`);
            throw new Error(`Static constructor parameter type type not yet implemented`)
        } else {
            logger.error(`Constructor parameter specs is neither "dynamic" nor "static`);
            throw new Error('Constructor parameter specs is neither "dynamic" nor "static');
        }
    });

    return parameters;
    // logger.debug(`The retrieved parameters are: ${parameters}`);
}
