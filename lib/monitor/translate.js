...
class DCRTranslator {
  ...
  getDCRFromTX(tx, activities) {
    ...
    transLogger.debug(`our method is: ${method}`);
    ...
    if (decodedParams.__length__ > 0) {
      for (const [key, value] of Object.entries(decodedParams)) {
        transLogger.debug(chalk.cyan(`key: ${key}, value: ${value}`));
        transLogger.debug(chalk.green(`this.modelFunctionParams: ${this.modelFunctionParams}`))
