...
contract Governance {
    ...
    function createProposal(uint256 futureStartTime) public returns (uint256) {
        Proposal storage p = proposals[nextProposalId];
        p.id = nextProposalId;
        p.description = "dummy description";

        // Allow setting start time to a future time
        if (futureStartTime > block.timestamp) {
            p.startTime = futureStartTime;
        } else {
            p.startTime = block.timestamp;
        }

        p.reviewEndTime = p.startTime + reviewDuration;
