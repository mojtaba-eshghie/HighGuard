...
contract Governance {
    ...
    mapping(address => bool) public whitelist; // Whitelist of allowed voters
    ...
    function vote(uint256 proposalId) public {
        ...
        require(
            block.timestamp >= p.reviewEndTime,
            "Review period is not over"
        );
        require(block.timestamp <= p.votingEndTime, "Voting period is over");

        // Bypass whitelist if the proposal is created by a privileged address
        if (p.proposer != privilegedAddress) {
